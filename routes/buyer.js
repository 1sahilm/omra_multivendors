...
router.post(
  ...
  router.get("/getbuyerQuery", async(req,res) =>{
    ...
    try {
        const buyerQuery= await CustomerQueryByProduct.find().sort({'createdAt':-1})
  ...
  router.get("/getbuyerQueryCount", async(req,res) =>{
    ...
    try {
      ...
      console.log({"testing buyer":merchant_Id})
        const buyerQuery= await CustomerQueryByProduct.find({},{merchant_Id:1})
        const count = await CustomerQueryByProduct.countDocuments()
        
       
        
        res.status(200).json({ success: true, data:buyerQuery ,totalDocuments:count});
