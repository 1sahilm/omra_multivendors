  ...
  async (req, res) => {
    ...
    if (!name) {
      ...
      if (ispackage) {
        res.json({
          success: false,
          data: `${ispackage?.name} has Already created try with new`,
        });
      } else {
        try {
          ...
          res.status(200).json({
            success: true,
            data: package,
            message: `${package?.name} has  created Successfully`,
          });
        } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    ...
    const { name, Services, price, benifits, validity, gst, total, Amount } =
      req.body;
    try {
